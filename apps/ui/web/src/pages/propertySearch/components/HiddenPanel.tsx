...
/**
 * Everything the user has hidden, with a way back. Hiding filters at render
 * time rather than deleting, so an unhide restores the card or row on the next
 * render without re-running the search.
 */
export default function HiddenPanel({ hidden, isLoading, error, onUnhide }: HiddenPanelProps) {
  return (
    <Card className='p-5'>
      ...
      {isLoading ? (
        <div className='flex items-center gap-2 py-6 text-ink-3'>
          <Spinner size='sm' />
          <p className='text-sm'>Loading hidden items</p>
        </div>
      ) : (
                    ...
                    {entries.map(entity => (
                      ...
                      >
                        ...
                        <Button
                          variant='ghost'
                          size='sm'
                          onClick={() => onUnhide(entity.entityKey)}
                        >
                          <Eye size={16} />
                          Unhide
                        </Button>
